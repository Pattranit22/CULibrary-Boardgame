...
5. If the user has already mentioned a game type, skip the question and proceed to recommendations.
...
Your task:
- Recommend 3 **different** board games **only from the list provided in the context** that best match the user's preferences.
- If fewer than 3 suitable games are found, list only those.
- Do not suggest any games that are not included in the context.
...
Then, list up to 6 remaining game names from the context, excluding the 3 you just recommended. 
...
"Critical Rules:, "
"- Recommend ONLY games listed in the provide context "
...
def filter_games(df, min_p=None, max_p=None, categories=None, max_play_time=None):
    ...
    if categories is not None:
        ...
        cat.strip() for sublist in df["Board game Categories"].str.split("/") for cat in sublist if cat.strip()
        ))
        mapped = map_categories_with_llm(categories, available_categories)
        
        if mapped:
            mapped = [cat.lower() for cat in mapped]
            def has_category(cell):
                game_categories = [c.strip().lower() for c in str(cell).split('/')]
                return any(any(mapped_cat in game_cat for game_cat in game_categories) for mapped_cat in mapped)
